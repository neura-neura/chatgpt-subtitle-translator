...
Fork maintained by [neura-neura](https://github.com/neura-neura). Original upstream: [Cerlancism/chatgpt-subtitle-translator](https://github.com/Cerlancism/chatgpt-subtitle-translator).
...
## Web Interface: <https://neura-neura.github.io/chatgpt-subtitle-translator/>  
...
## Fork Notes: neura-neura Web UI changes

This fork adds a GitHub Pages oriented Ollama workflow and several quality-of-life improvements to the web app.
...
- The page includes a detailed first-run Ollama guide for Windows, including:
  - installation from PowerShell and `cmd`
  - `ollama pull` for the default model
  - start and stop commands for Ollama
  - `OLLAMA_ORIGINS` setup for GitHub Pages
- `Test Ollama Connection` checks the configured base URL and confirms whether the selected model is installed.
- The GitHub Pages warning card is dismissible and can be permanently hidden with `localStorage`.

### Saved settings and instruction helpers
- `To Language` is persisted in `localStorage`.
- `System Instruction` is persisted in `localStorage`.
- The `Model` field is an editable dropdown with two built-in defaults and up to five custom recent models stored in `localStorage`.
- Saved system instruction presets can be stored locally with title and description metadata.
- The saved instruction library is collapsed by default.
- Clicking `Use` replaces the current `System Instruction` with the saved text.
- `Save Current Instruction` supports the title-first flow from the current instruction text.

### Export naming and merge workflow
- Exported translated `.srt` files keep the original imported file name and append the target language suffix.
- The web UI includes a `Merge Bilingual` split button beside `Export SRT`.
- Merge defaults:
  - top subtitle: the imported source subtitle
  - bottom subtitle: the latest translated subtitle generated by the app
- The dropdown also lets you choose custom top and bottom subtitle files from your computer.
- The merge algorithm was ported to JavaScript from the local Python combiner script and preserves the fuzzy overlap matching behavior used in the fork.
- The `{\\rENG}` cleanup logic was also ported from the local Python script.
- The merge dropdown includes a persisted `Keep {\\rENG} tag` toggle.
- That toggle is off by default, so combined exports remove `{\\rENG}` unless you explicitly enable it.
- The current `{\\rENG}` mode is shown below the merge button even when the dropdown is closed.
- Combined export names follow the imported base file name plus language suffixes, and append `_no_rENG` when the tag is removed.
- If `From Language` is empty, the combined file name uses `to_<target-language>` instead of `source_<target-language>`.
- Merge completion feedback is shown as a dismissible toast instead of shifting the action bar layout.

### Branding
- The web footer now shows the upstream author and also identifies this site as the fork by `@neura-neura`.
...
- Clone this repository
  ```bash
  git clone https://github.com/neura-neura/chatgpt-subtitle-translator
  ```
